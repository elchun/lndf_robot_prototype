"""
Test bed for playing with occupancy network
"""
...
from ndf_robot.eval.ndf_occ import NDFOcc

from ndf_robot.utils import path_util
import ndf_robot.model.vnn_occupancy_net_pointnet_dgcnn as vnn_occupancy_network
from ndf_robot.eval.ndf_occ_alignment import NDFAlignmentCheck
...
if __name__ == '__main__':
    ...
    sigma = 0.30 
    n_test_points = 1000
    test_points = np.random.normal(0.0, sigma, size=(n_test_points, 3))

    ndf_occ = NDFOcc(model)
    occ = ndf_occ.test_point(pcd1[None, :, :], test_points[None, :, :])
    ...
    m = np.ma.masked_where(occ > 0, occ)
    print(m)
