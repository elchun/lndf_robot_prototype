...
if __name__ == '__main__':
    ...
    # -- Objects for graphical abstract -- #
    # obj_model = osp.join(path_util.get_ndf_obj_descriptions(), 'mug_centered_obj_normalized/  /models/model_normalized.obj')
    ...
    def generate_object(obj_model, shapenet_id, second_rot=None):
        scale = 1.0
        # scale = 0.8
        ...
        if second_rot is not None:
            mesh.apply_transform(second_rot)
        ...
        fname = osp.join(path_util.get_ndf_eval(), 'debug_viz', f'grid_fig_{shapenet_id}.html')

        fig.write_html(fname)

    # base_dir = osp.join(path_util.get_ndf_obj_descriptions(), 'bottle_handle_centered_obj_normalized')
    # base_dir = osp.join(path_util.get_ndf_obj_descriptions(), 'mug_std_centered_obj_normalized')
    # for i, shapenet_id in enumerate(os.listdir(base_dir)):
    #     object_model = osp.join(base_dir, shapenet_id, 'models/model_normalized.obj')
    #     generate_object(object_model, shapenet_id)
    #     if i > 20:
    #         print('Done')
    #         break

    # obj_model = './teapot.obj'
    # fname = 'teapot'
    fname = 'e824b049f16b29f19ab27ff78a8ea481-h_rot'
    obj_model = osp.join(path_util.get_ndf_obj_descriptions(), 'bottle_handle_centered_obj_normalized/e824b049f16b29f19ab27ff78a8ea481-h/models/model_normalized.obj')

    rot_ori = np.eye(4)
    rot_ori[:3, :3] = util.make_rotation_matrix('x', -np.pi / 2)
    rot = np.eye(4)
    rot[:3, :3] = util.make_rotation_matrix('z', np.pi / 5)
    rot = rot @ rot_ori
    # rot1 = np.eye(4)
    # rot1[:3, :3] = R.random().as_matrix()
    generate_object(obj_model, fname, rot)
